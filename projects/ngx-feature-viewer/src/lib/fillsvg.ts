...
class PreComputing {
    ...
    public preComputingLine(object) {
      ...
      const shift = parseInt(object.height);
      let level = 0;
...
class FillSVG extends ComputingFunctions {
    ...
    private isLight(mycolor) {
      const color = mycolor.match(/^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+(?:\.\d+)?))?\)$/);
      const r = color[1];
      const g = color[2];
      const b = color[3];
    ...
    private shadeBlendConvert(p, from= '#000', to= null) {
      ...
      const f = this.sbcRip(from, i, r);
      const t = this.sbcRip(to, i, r);
      if (!f || !t) {
        return null;
      } // ErrorCheck
      if (h) {
    ...
    public typeIdentifier(feature) {
      ...
      if (feature.type === 'curve') {
        ...
        const shift = parseInt(feature.height);
...
console.log(feature)
      ...
      if (feature.type === 'rect') {
        ...
        const seq = this.displaySequence(this.commons.current_extend.length);
        if (seq === false) {
          this.sequenceLine();
        } else if (seq === true) {
    ...
    public tagArea(object, thisYPosition) {
      ...
      const id = 't' + object.id + '_tagarea';
      const featureArea = this.commons.tagsContainer.append('g')
          .attr('class', 'tagGroup')
          .attr('id', id)
        ...
        for (const bt of object.sidebar) {
          if (bt.type) {
            if (bt.type !== 'button' && bt.type !== 'percentage' && bt.type !== 'link' && bt.type !== 'icon') {
              ...
              let content;
              if (bt.type == 'button') {
                const cl = this.hexToRgb(this.commons.viewerOptions.flagColor);
                const col = 'rgba(' + [cl.b, cl.g, cl.r].join(',') + ')';
                const colalph = 'rgba(' + [cl.b, cl.g, cl.r].join(',') + ',0.8)';
                const coltext = this.isLight(col) ? 'black' : 'white';
                ...
                const disordersString = bt.label.toString() + '%';
                const colorrgb = this.hexToRgb(this.gradientColor(bt.label));
                const color = 'rgba(' + [colorrgb.r, colorrgb.g, colorrgb.b].join(',') + ')';
              ...
              objectPos += (D3Selection.select('#' + bt.id).node() as HTMLElement).getBoundingClientRect().width + 3;
            }
          } else if (bt.content) {
            const gHtml = featureArea
                .append('g')
                .attr('id', id + '_button_' + bt.id)
            ...
            try {
              const contentwidth = (D3Selection.select(`#${bt.id}`).select('*').node() as HTMLElement).getBoundingClientRect().width;
    ...
    public sequence(seq, start = 0) {
      ...
      const sequenceAAs = this.commons.svgContainer.append('g')
    ...
    public sequenceLine() {
      ...
      if (this.commons.viewerOptions.dottedSequence) {
        const dottedSeqLine = this.commons.svgContainer.selectAll('.sequenceLine')
    ...
    public rectangle(object, position) {
      ...
      const rectHeight = this.commons.elementHeight;

      const rectShift = rectHeight + rectHeight / 3;
      const lineShift = rectHeight / 2 - 6;
      position = Number(position) + 3; // center line

      const rectsPro = this.commons.svgContainer.append('g')
      ...
      const rectsProGroup = rectsPro.selectAll('.' + object.className + 'Group')
          .data(object.data)
          .enter()
          .append('g')
      ...
      rectsProGroup
          ...
          .attr('id', (d) => {
            ...
            if (d.id) {
              ...
              // add id to object
              const id = 'f_' + object.id + '_' + d.x + '-' + d.y;
              d.id = id;
              return id;
            }
          })
      ...
      const uniqueShift = rectHeight > 12 ? rectHeight - 6 : 0;
    ...
    public lollipop(object, position) {
      const circlesPro = this.commons.svgContainer.append('g')
    ...
    public path(object, position) {
      ...
      const pathHeight = this.commons.elementHeight;

      const pathsDB = this.commons.svgContainer.append('g')
    ...
    public arrow(object, position) {
      ...
      const rectsProGroup = rectsPro.selectAll('.' + object.className + 'Group')
          ...
          .attr('class', object.className + 'Group')
          .attr('transform', (d) => {
            let transform = 'translate(' + this.rectX(d) + ',0)';
            if (d.direction === 'left') {
              transform += ' scale(-1, 1)';
            }
            return transform;
    ...
    public fillSVGLine(object, position = 0) {
      ...
      const histoG = this.commons.svgContainer.append('g')
    ...
    public multipleRect(object, position = 0, level = this.commons.level) {
      ...
      const rects = this.commons.svgContainer.append('g')
    ...
    public addXAxis(position) {
      ...
      if (!this.commons.viewerOptions.showAxis) {
        D3Selection.select(`#${this.commons.divId}`).selectAll('.tick')
            .attr('display', 'none');
      }
    }
    ...
    public resizeBrush() {
      if (this.commons.svgContainer) {
        if (this.commons.svgContainer.node() !== null) {
          const rectArea = this.commons.svgContainer.node().getBoundingClientRect();
          const thisbrush = this.commons.svgContainer.select('.brush');
    ...
    public showHelp() {
      ...
      const helpContent = 'To zoom in : Left click to select area of interest\n To zoom out : Right click to reset the scale\n Zoom max : Limited to ' +
