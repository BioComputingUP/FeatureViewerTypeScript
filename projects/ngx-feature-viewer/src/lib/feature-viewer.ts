import {D3Selection, D3Scale, D3Shape, D3Axis, D3Brush} from './custom-d3';
...
class FeatureViewer {
  ...
  private parseUserOptions(options: UserOptions): void {
    ...
    this.commons.viewerOptions.labelTrackWidthMobile = 130;
    if (options && options.flagTrackMobile) {
      if (typeof options.flagTrackMobile === 'string') {
        ...
        this.commons.viewerOptions.labelTrackWidthMobile = options.flagTrackMobile ? 130 : 0;
      } else {
        this.commons.viewerOptions.labelTrackWidthMobile = 130;
        this.commons.logger.warn(`Automatically set tagsTrackWidth to ${this.commons.viewerOptions.tagsTrackWidth}`, {fvId:this.divId});
      }
    ...
    let myd3node = D3Selection.select(`#${this.divId}`).node();
    if (myd3node) {
    ...
    let myvod3node = D3Selection.select(`#${this.divId}`).node();
    if (myvod3node) {
  ...
  private updateYAxis() {
    ...
    this.commons.yAxisSVGGroup = this.commons.yAxisSVG
      ...
      .on('mouseover', (d) => {
        if (this.commons.viewerOptions.showSubFeatures && d.hasSubFeatures) {
          D3Selection.select(`#${this.divId}`).select(`#${d.id}`).selectAll(".Arrow").style("fill-opacity", 0.9);
        }
      })
      .on('mouseout', (d) => {
        if (this.commons.viewerOptions.showSubFeatures && d.hasSubFeatures) {
          D3Selection.select(`#${this.divId}`).select(`#${d.id}`).selectAll(".Arrow").style("fill-opacity", 0.6);
        }
      })
  ...
  private brushend() {
    ...
    if (this.commons.featureSelected) {
      D3Selection.select(`#${this.divId}`).select(`#${this.commons.featureSelected}`).style("fill-opacity", "0.6");
    ...
    if (currentEvent.sourceEvent !== null && typeof currentEvent.sourceEvent.target !== "function") {
      ...
      if (this.commons.extent) { // zooming
        ...
        if (extentLength > this.commons.viewerOptions.zoomMax) {
          ...
          D3Selection.select(`#${this.divId}`).select(".zoomUnit")
      ...
      D3Selection.select(`#${this.divId}`).select(".brush").call(this.commons.brush.move, null);
  ...
  private resizeForMobile() {
    // change flags
    let flags = D3Selection.select(`#${this.divId}`).selectAll(".Arrow")
      .attr("points", (d) => {
        // match points with subFeature level
  ...
  private updateWindow() {
    // change width now
    if (D3Selection.select(`#${this.divId}`).node() !== null) {

      let d3node = D3Selection.select(`#${this.commons.divId}`).node();
      ...
      if (this.commons.viewerOptions.brushActive) {
        D3Selection.select(`${this.commons.divId} .background`).attr("width", this.commons.viewerOptions.width - 10);
      }

      // resize brush
      D3Selection.select(`#${this.commons.divId}`).select(".fvbrush").call(this.commons.brush.move, null);
      ...
      if (this.commons.animation) {
        // @ts-ignore
        D3Selection.select(`#${this.commons.divId}`).select('#tags_container').transition().duration(500)
      }
      D3Selection.select(`#${this.commons.divId}`).select('#tags_container')
        .attr("transform", "translate(" + (this.commons.viewerOptions.margin.left + this.commons.viewerOptions.width + 10).toString() + ",10)");

      this.transition_data(this.commons.features, this.commons._current_extend?.start);
      this.fillSVG.reset_axis();
  ...
  private recursivelyRemove(ft) {
    ...
    D3Selection.select(`#t${ft.id}_tagarea`).remove();
    D3Selection.select(`#c${ft.id}_container`).remove();
    D3Selection.select(`#${ft.id}`).remove();
  }
  ...
  private init(div) {
    ...
    this.commons.scaling = D3Scale.scaleLinear()
    ...
    this.commons.scalingPosition = D3Scale.scaleLinear()
    ...
    D3Selection.select(`#${this.divId}`)
      .append("foreignObject")
      .html(overlayhtml)

    this.commons.lineBond = D3Shape.line()
    ...
    this.commons.lineGen = D3Shape.line()
      .x((d) => {
        // return this.commons.scaling(d[0]);
        return this.commons.scaling(d['x']);
    ...
    this.commons.lineYScale = D3Scale.scaleLinear()
      .domain([0, -30])
      .range([0, -20]);

    this.commons.line = D3Shape.line()
    ...
    //Create Axis
    this.commons.xAxis = D3Axis.axisBottom(this.commons.scaling)
      .tickValues(tickArray)
    //.scale(this.commons.scaling) // TODO
    //.tickFormat(d3.format("d"));

    let yAxisScale = D3Scale.scaleBand()
      //.domain([0, this.commons.yData.length])
      .rangeRound([0, 500]);

    // Y Axis
    D3Axis.axisLeft(yAxisScale)
    ...
    this.commons.brush = D3Brush.brushX()
    ...
    this.commons.tooltipDiv = D3Selection.select(`#${this.divId}`)
      .append("div")
      .attr("class", "fvtooltip")
      .attr("id", "fvtooltip")
      .style("opacity", 0)
      .style("z-index", 1070);
    this.commons.customTooltipDiv = D3Selection.select(`#${this.divId}`)
      .append("div")
    ...
    this.commons.style = D3Selection.select(`#${this.divId}`)
    // .append("style")
    // .html(`${fvstyles}`)
    ...
    if (this.commons.viewerOptions.toolbar) {
      let headerOptions = document.querySelector(div + " .svgHeader") ? D3Selection.select(div + " .svgHeader") : D3Selection.select(div).append("div").attr("class", "svgHeader");
    ...
    this.commons.svg = D3Selection.select(div).append("svg")
    ...
    this.commons.svgElement = D3Selection.select(`#${this.divId}`).select('svg').node();
    ...
    this.commons.svgContainer.on('mousemove', () => {
      let absoluteMousePos = D3Selection.mouse(this.commons.svgContainer.node());
  ...
  public resetHighlight(resetLastHighlight=true) {
    ...
    // remove selection rectangle
    D3Selection.select(`#${this.divId}`).selectAll(".selectionRect").remove();
  ...
  public resetZoom() {
    this.resetHighlight()
    //reset scale
    D3Selection.select(`#${this.divId}`).select(".zoomUnit").text("1");
    ...
    D3Selection.select(`#${this.divId}`).select(".fvbrush").call(this.commons.brush.move, null);
  ...
  public clickFlag(d) {
    if (d) {
      // remove selected features
      if (this.commons.featureSelected) {
        D3Selection.select(`#${this.commons.divId}`).select(`#${this.commons.featureSelected}`).style("fill-opacity", "0.6");
        this.commons.featureSelected = null;
      }
      // remove selection rectangle
      D3Selection.select(`#${this.commons.divId}`).select(".selectionRect").remove();
  ...
  public emptyFeatures() {
    ...
    // transit svgContent
    let container = D3Selection.select(`#${this.divId} #svgContent`);
  ...
  public flagLoading(id) {
    D3Selection.select(`#${this.commons.divId}`).select("#fvoverlay").attr("class", "pageoverlay")
  };
  ...
  public stopFlagLoading = (id) => {
    D3Selection.select(`#${this.divId}`).select("#fvoverlay").attr("class", null)
  };
